 ...
 */
class ReplaceTicket extends CommonObject
{
    /**
    ...
    public function close(User $user, $mode = 0)
    {
        ...
        if ($this->status != Ticket::STATUS_CLOSED && $this->status != Ticket::STATUS_CANCELED)
        { // not closed
            $this->db->begin();
            ...
            if (!$error && $id > 0)
            {
                ...
                if (!empty($public_area))
                {
                    ...
                    if (!empty($conf->global->TICKET_PUBLIC_NOTIFICATION_NEW_MESSAGE_ENABLED))
                    {
                        ...
                        if (
                            !empty($conf->global->TICKET_NOTIFICATION_ALSO_MAIN_ADDRESS) &&
