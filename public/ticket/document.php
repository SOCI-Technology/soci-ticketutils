...
$object_ticket = new ActionsTicket($db);
$email = GETPOST('email');
$track_id = GETPOST('track_id');
$id_to_use = $conf->global->TICKETUTILS_ONLY_ONE_ID ? 'ref' : 'track_id';

$ret = ($id_to_use == 'ref' ? $object_ticket->fetch('', $track_id, '') : $object_ticket->fetch('', '', $track_id));

if ($ret && $object_ticket->dao->id > 0)
{
    // Check if emails provided is the one of author
    $emailofticket = CMailFile::getValidAddress($object_ticket->dao->origin_email, 2);
    ...
    else
    {
        ...
        $contacts = $object_ticket->dao->liste_contact(-1, 'external');
    ...
    if ($object_ticket->dao->fk_soc > 0 || $object_ticket->dao->socid > 0)
    {
        $object_ticket->dao->fetch_thirdparty();
        if ($email == $object_ticket->dao->thirdparty->email)
        {
            $display_ticket = true;
            $_SESSION['email_customer'] = $email;
        }
    ...
    if ($object_ticket->dao->fk_user_create > 0)
    {
        $tmpuser = new User($db);
        $tmpuser->fetch($object_ticket->dao->fk_user_create);
    ...
    // Check if email is email of creator
    if ($object_ticket->dao->fk_user_assign > 0 && $object_ticket->dao->fk_user_assign != $object_ticket->dao->fk_user_create)
    {
        $tmpuser = new User($db);
        $tmpuser->fetch($object_ticket->dao->fk_user_assign);
        if (strtolower($email) == strtolower($tmpuser->email))
        {
            $display_ticket = true;
            $_SESSION['email_customer'] = $email;
...
$object_ticket = new stdClass();
$reshook = $hookmanager->executeHooks('downloadDocument', $parameters, $object_ticket, $action); // Note that $action and $object may have been
if ($reshook < 0)
{
