...
$hookmanager->initHooks(array('ticketutils_ticketpubliclist', 'globalcard'));
			...
			if ($resql)
			{
				...
				while ($obj = $db->fetch_object($resql))
				{
					...
					if (!empty($arrayfields['t.ref']['checked']))
					{
						...
						print '<a rel="nofollow" href="javascript:viewticket(\'' . dol_escape_js($obj->$id_to_use) . '\',\'' . dol_escape_js($_SESSION['email_customer']) . '\');">';
				...
				print '<form method="post" id="form_view_ticket" name="form_view_ticket" action="' . dol_buildpath('/custom/ticketutils/public/ticket/view.php', 1) . (!empty($entity) && isModEnabled('multicompany') ? '?entity=' . $entity : '') . '" style="display:none;">';
				print '<input type="hidden" name="token" value="' . newToken() . '">';
