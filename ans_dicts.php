...
if (!GETPOSTISSET('search_country_id') && $search_country_id == '' && ($id == 2 || $id == 3 || $id == 10))
{	// Not a so good idea to force on current country for all dictionaries. Some tables have entries that are for all countries, we must be able to see them, so this is done for dedicated dictionaries only.
...
if (empty($reshook))
{
	...
	if (GETPOST('actionadd') || GETPOST('actionmodify'))
	{
		...
		if ($id == 10 && GETPOSTISSET("code"))
		{  // Spaces are not allowed into code for tax dictionary
...
echo dol_get_fiche_head($head, $dict, $title, -1, 'ticket');
...
function fieldList($fieldlist, $obj = '', $tabname = '', $context = '')
{
	...
	foreach ($fieldlist as $field => $value)
	{
		...
		elseif ($value == 'country_id')
		{
			...
			{	// If there is already a field country, we don't show country_id (avoid duplicate)
		...
		elseif (in_array($value, array('element', 'source')))
		{	// Example: the type and source of the element (for contact types)
			$tmparray = array();
			if ($value == 'element')
			{
